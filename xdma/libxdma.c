...
static irqreturn_t xdma_isr(int irq, void *dev_id)
{
	...
	//clear irq
	user_irq &= 0xff;
	write_register(user_irq, user_intc_reg + 0xb0, REGS_USER_OFS_INT_V2 + 0xb0);
	...
	if (user_irq) {
		...
		//input_report_key(xdev->input_dev, KEY_0, 1);
		//input_sync(xdev->input_dev);
		//input_report_key(xdev->input_dev, KEY_0, 0);
		//input_sync(xdev->input_dev);

		printk(KERN_ERR "send_timestamp = %llu.%09llu(s)\n", timestamp/1000000000, timestamp%1000000000);
