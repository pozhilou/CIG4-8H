...
static int probe_one(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	xpdev->xdev->input_dev = input_allocate_device();
	if(xpdev->xdev->input_dev == NULL)
		printk(KERN_ERR "Failed to allocate input device.");
	xpdev->xdev->input_dev->name = "key_inputdev";
	__set_bit(EV_KEY, xpdev->xdev->input_dev->evbit);
	__set_bit(KEY_0, xpdev->xdev->input_dev->keybit);

	ret = input_register_device(xpdev->xdev->input_dev);  //注册input_dev
	if(ret < 0)
...
static void remove_one(struct pci_dev *pdev)
{
	...
	input_unregister_device(xpdev->xdev->input_dev);
	input_free_device(xpdev->xdev->input_dev);
